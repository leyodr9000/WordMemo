...
### v0.5.6（修复 MIUI X 模式点击下拉闪退）
- ✅ 修复「设置 → 学习/外观」点击下拉选项（SuperDropdown）闪退的问题：弹层改**就地渲染**（`renderInRootScaffold=false`），绕开 root-scaffold 挂载路径与界面切换/嵌套场景的冲突
- ✅ 外观节「界面风格」由下拉改为**即时按钮**（点击即切、无弹层）—— 切换界面风格会重建整个界面，弹层与界面重建共存必然崩溃
- ✅ 词书页加号菜单弹层同步就地渲染

### v0.5.5（细节打磨 + 多张排队导入）
- ✅ 启动器图标内容放大（源图裁掉外围 24%，蒙版内显示范围更大）
- ✅ 设置-外观：两个取色行增加垂直间距
- ✅ 设置-学习：每日目标仅保留进度条（移除 10/20/30/50 快捷按钮）
- ✅ 翻译源/卡片切换动画/发音音色/界面风格改为 Miuix SuperDropdown（HyperOS 下拉单选，替换 M3 灰色 chips）
- ✅ 卡片切词动画改为 AnimatedContent 滑动切换（点击「已掌握」等按钮必定触发，方向驱动，替代原低触发率方案）
- ✅ Miuix 模式首页搜索框放大镜间距修正；**搜索时自动收起首页默认内容**（输入法不再遮挡，无匹配时给提示）
- ✅ 词书页加号菜单改为 Miuix ListPopup 弹层（HyperOS 风格）
- ✅ **拍照导入支持多张照片**：相册多选 → 排队逐张识别（显示第 x/n 张与阶段），结果汇总去重统一预览导入；单张失败不中断队列并汇总提示
- ✅ 阅读页：全文翻译开关换 Miuix Switch；修复「关闭全文翻译后翻译仍显示」的问题（区分全文/单句翻译来源，关闭时隐藏全文产生的翻译，重开不重复计费）
